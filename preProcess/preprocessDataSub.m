...
% JIC -- parameters for the filter now set here, to avoid repeating in every loop
% filter [a,b] = ops.filterParam, if no filter ops.doFilter = 0

if ops.doFilter == 1
    if isfield(ops,'fslow') && ops.fslow<ops.fs/2 && isfield(ops,'fshigh')
        % valid value for low pass and high pass filters, create butterworth bp
        [ops.filtb1, ops.filta1] = butter(3, [ops.fshigh/ops.fs,ops.fslow/ops.fs]*2, 'bandpass'); % butterworth filter with only 3 nodes (otherwise it's unstable for float32)
    elseif isfield(ops,'fslow') && ops.fslow<ops.fs/2 && ~isfield(ops,'fshigh')
        % low pass only (unusual)
        [ops.filtb1, ops.filta1] = butter(3, (ops.fshigh/ops.fs)*2, 'low');
    elseif isfield(ops,'fshigh')
        % highpass filter only; the default is to only do high-pass filtering at 150Hz
        fprintf('set filter params\n')
        [ops.filtb1, ops.filta1] = butter(3, (ops.fshigh/ops.fs)*2, 'high');
    else
        % no valid filter
        ops.doFilter = 0;
    end
...
% save the filtering params into rez.ops
rez.ops = ops;
...
res.ops = ops; % to record filter parameters
